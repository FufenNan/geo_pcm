...
class MixDataset(Dataset):
    ...
    def load_hypersim(self):
        ...
        for scene_dir in tqdm(scene_dirs, desc='Loading HyperSim'):
            final_preview_dir = os.path.join(scene_dir, 'images', 'scene_cam_*_final_preview')
            geometry_preview_dir = os.path.join(scene_dir, 'images', 'scene_cam_*_geometry_preview')
            for image_dir in glob.glob(final_preview_dir):
                color_files = glob.glob(os.path.join(image_dir, 'frame.*.color.jpg'))
                for color_file in color_files:
                    base_name = os.path.basename(color_file).replace('.color.jpg', '')

                    depth_file = os.path.join(image_dir.replace('final_preview', 'geometry_preview'), f'{base_name}.depth_meters.png')
                    normal_file = os.path.join(image_dir.replace('final_preview', 'geometry_preview'), f'{base_name}.normal_cam.png')

                    # Albedo and Shading files in the final_hdf5 folder
                    albedo_file = os.path.join(image_dir, f'{base_name}.diffuse_reflectance.jpg')
                    shading_file = os.path.join(image_dir, f'{base_name}.diffuse_illumination.jpg')
    ...
    def __getitem__(self, index):
        ...
        if sample_path['dataset'] == 'hypersim':
            ...
            sample['albedo'] = cv2.imread(sample_path['albedo'])[:, :, ::-1].astype(np.float32)  # [H, W, 3]
